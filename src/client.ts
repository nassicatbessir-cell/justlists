...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env["DEFAULT_TEAM_4_HOHQ_BASE_URL"].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env["DEFAULT_TEAM_4_HOHQ_LOG"] or 'warn' if it isn't set.
   */
...
export type DemoAPIScalarGalaxyOptions = ClientOptions;

/**
 * API Client for interfacing with the DemoApiScalarGalaxy API.
 */
export class DemoAPIScalarGalaxy {
   ...
   * @param {string | null | undefined} [opts.webhookSecret=process.env["DEFAULT_TEAM_4_HOHQ_WEBHOOK_SECRET"] ?? null]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env["DEFAULT_TEAM_4_HOHQ_BASE_URL"] ?? https://galaxy.scalar.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('DEFAULT_TEAM_4_HOHQ_BASE_URL'),
    ...
    webhookSecret = readEnv('DEFAULT_TEAM_4_HOHQ_WEBHOOK_SECRET') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    if (baseURLOverridden && options.environment)
      throw new Errors.DemoAPIScalarGalaxyError(
        'Ambiguous URL; The `baseURL` option (or DEFAULT_TEAM_4_HOHQ_BASE_URL env var) and the `environment` option are given. If you want to use the environment you must pass baseURL: null',
      );
    const defaultBaseURL = environments[environment];
    this.baseURL = options.baseURL || defaultBaseURL;
    this.timeout = options.timeout ?? DemoAPIScalarGalaxy.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('DEFAULT_TEAM_4_HOHQ_LOG'), 'process.env["DEFAULT_TEAM_4_HOHQ_LOG"]', this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('DEFAULT_TEAM_4_HOHQ_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  private async resolveAuthOption(
    ...
    if (!token)
      throw new Errors.DemoAPIScalarGalaxyError(`Expected '${optionName}' to resolve to a non-empty string.`);
    return token;
  }
  ...
  static DemoAPIScalarGalaxy = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static DemoAPIScalarGalaxyError = Errors.DemoAPIScalarGalaxyError;
...
DemoAPIScalarGalaxy.Planets = Planets;
DemoAPIScalarGalaxy.CelestialBodies = CelestialBodies;
DemoAPIScalarGalaxy.Authentication = Authentication;
DemoAPIScalarGalaxy.Webhooks = Webhooks;

export declare namespace DemoAPIScalarGalaxy {
